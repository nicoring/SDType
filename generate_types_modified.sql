...
FROM prefix_dbpedia_untyped_instance as instance
...
INSERT INTO en_resulting_types 
SELECT resource,type,SUM(tf*percentage*weight)/SUM(tf*weight) AS score
FROM en_stat_resource_predicate_type 
GROUP BY resource,type 
HAVING score>=0.05;

# Read types at the threshold you like, e.g.
INSERT INTO de_resulting_types_readable
SELECT r2md5.resource,t2md5.type, res.score FROM de_resulting_types AS res
LEFT JOIN de_dbpedia_resource_to_md5 AS r2md5 ON res.resource=r2md5.resource_md5
LEFT JOIN de_dbpedia_type_to_md5 AS t2md5 ON res.type=t2md5.type_md5;
WHERE score>=0.4